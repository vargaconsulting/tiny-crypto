...
@testset "ECPoint operator overloads and infinity behavior" begin
    #for T in (Int, UInt8, UInt16, UInt32, UInt64, UInt128, BigInt)

    for T in (UInt8,)
        C = Weierstrass{T}(30:103, 0:2, 6:100)
        #C = Weierstrass{UInt8}(97, 2, 3, 101, Point{UInt8}(3, 6))
        G = ECPoint(C.G, C)
        O = ECPoint(infinity(T), C)  # or simply ∞(Int)
